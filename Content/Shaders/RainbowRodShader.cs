...
namespace CelestialMod.Content.Shaders;


[StructLayout(LayoutKind.Sequential, Size = 1)]
public struct Waves
{
	private static VertexStripWaves _vertexStrip = new VertexStripWaves();
	...
	public void Draw(Projectile proj)
	{
		...
		Waves._vertexStrip.PrepareStripWithProceduralPadding(proj.oldPos, proj.oldRot, StripColors, StripWidth, -Main.screenPosition + proj.Size / 4f);
		Waves._vertexStrip.DrawTrail();
...
public class VertexStripWaves
{
	...
	private void AddVertex(StripColorFunction colorFunction, StripHalfWidthFunction widthFunction, Vector2 pos, float rot, int indexOnVertexArray, float progressOnStrip)
	{
		...
		{
			Array.Resize(ref this._vertices, this._vertices.Length * 4);
		}
